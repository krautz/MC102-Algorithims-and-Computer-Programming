...
int main () {
     ...
     * para for's. aux_1 e aux_2 auxiliares para guardar informacoes sobre posicoes valiosas. flag e flag_2 usadas para encerrar um while quando necessario.
    ...
    int jogadores, times, i, j, k, *capitao, aux_1, aux_2, flag = 1, flag_2 = 1, **time, m = 1;
    ...
    for (i = 0; i < times; i++) {
         ...
         * proximo capitao e colocamos ele como 1o jogador de seu time */
    ...
    while (flag) {
        ...
        for (i = 0, flag_2 = 1; i < jogadores && flag_2; i++) {
            for (j = 0; j < jogadores && flag_2; j++) {
                if (notas[i][j] != -1 && flag_2)
                    flag_2 = 0;
            }
        }
        if (flag_2)
